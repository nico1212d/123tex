...
def setup_logging() -> None:
    ...
    log_file = log_dir / f"main_{time.strftime('%Y%m%d_%H%M%S')}.log"
    ...
    try:
        ...
        for python_path in possible_paths:
            if python_path.exists() and python_path.is_file():
                logging.info(f"找到Python解释器: {python_path}")
                return python_path
        
        logging.error("未找到可用的Python解释器")
        return None
        
    except Exception as e:
        logging.error(f"获取Python解释器路径时出错: {e}")
        return None
...
def is_first_run() -> bool:
    ...
    try:
        ...
        if not marker.exists():
            ...
            marker.touch()  # 创建空文件
            logging.info("检测到首次运行，已创建标记文件")
            return True
        
        logging.info("检测到非首次运行")
        return False
        
    except Exception as e:
        logging.error(f"检查首次运行状态时出错: {e}")
...
def run_python_script(script_name: str) -> bool:
    ...
    try:
        ...
        if not target_script.exists():
            logging.error(f"目标脚本不存在: {target_script}")
            return False
        
        # 获取Python解释器路径
        python_path = get_python_interpreter()
        if python_path is None:
            logging.error("无法找到Python解释器")
            return False
        ...
        logging.error(f"脚本执行超时: {script_name}")
        return False
    except FileNotFoundError as e:
        logging.error(f"文件未找到: {e}")
        return False
    except Exception as e:
        logging.error(f"执行脚本时出错: {script_name}, 错误: {e}")
        return False
...
def safe_system_command(command: str, timeout: int = 30) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            logging.info(f"系统命令执行成功: {command}")
            return True
        else:
            logging.warning(f"系统命令执行失败: {command}, 返回码: {result.returncode}")
            return False
            
    except subprocess.TimeoutExpired:
        logging.error(f"系统命令执行超时: {command}")
        return False
    except Exception as e:
        logging.error(f"执行系统命令时出错: {command}, 错误: {e}")
        return False
...
def check_dir_legal() -> bool:
    ...
    try:
        ...
        if has_chinese:
            ...
            logging.error("程序因路径包含特殊字符而退出")
            return True
        else:
            logging.info(f"路径检查通过: {current_path}")
            return False
...
def main() -> None:
    ...
    try:
        # 设置日志
        setup_logging()
        logging.info("MaiBot 一键包启动")
        
        # 检查目录路径合法性
        if check_dir_legal():
            logging.error("目录路径不合法，程序退出")
            sys.exit(1)
        
        # 检查是否首次运行
        if is_first_run():
            # 初始化一键包
            logging.info("首次运行一键包，执行初始化操作")
            print("首次运行一键包，执行初始化操作……")
            
            if not run_python_script("update_modules.py"):
                logging.error("模块更新失败")
                return
            ...
            if not run_python_script("main.py"):
                logging.error("MaiBot启动失败")
                return
        else:
            # 非首次运行
            logging.info("检测到不是首次运行，正在跳过向导启动 MaiBot Core")
            print("检测到不是首次运行，正在跳过向导启动 MaiBot Core...")
            
            if not run_python_script("start.py"):
                logging.error("启动主程序失败")
                return
        ...
        logging.error(f"程序执行过程中出现未知错误: {e}")
        print(f"程序执行失败: {e}")
        sys.exit(1)
